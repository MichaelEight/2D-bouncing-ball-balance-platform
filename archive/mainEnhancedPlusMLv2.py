...
# Function to save the model, iteration number, and data
...
def load_model():
    ...
    if latest_model:
        ...
        return loaded_model, loaded_iteration  # Return the loaded model and iteration number
    else:
        print("No saved models found.")
        return None, None
...
def reset_simulation():
    ...
    global local_time, lastTime, deltaTime, timeSinceLastHit, time_spent_around_target
    global sim_iteration, showStartingInfo
    global total_distance_covered, simulation_start_time, targetX
    global data, current_iteration, total_reward_per_iteration
    ...
    print(f"Simulation Time: {simulation_time}, Total Distance: {total_distance_covered}, Score: {total_reward_per_iteration}")
...
def calculate_reward(distance_to_target_x, time_elapsed, angle):
    # Define parameters to tune the reward calculation
    distance_weight = 0.1  # Weight for distance covered
    time_weight = 0.0      # Weight for time spent without losing the ball
    time_penalty = 20.0     # Penalty for losing the ball
    angle_overuse_penalty = 0.0 # Penalty for using too steep angles 
    ...
    # Reward based on distance covered (negative reward)
    distance_reward = -distance_weight * distance_to_target_x

    # Reward for time spent without losing the ball
    isBallControlLost = (ballPosition[0] < p1[0] and ballVelocity[0] <= 0) or (ballPosition[0] > p2[0] and ballVelocity[0] >= 0)
    if not isBallControlLost:
        time_reward = time_weight * time_elapsed
    else:
        # time_reward = -time_penalty*abs(time_limit-time_elapsed)
        time_reward = -time_penalty
    
    # if abs(angle) >= 0.9 * limit_phi:
    #     angle_reward = -angle_overuse_penalty * angle
    # else:
    #     angle_reward = 0.0

    if abs(angle) <= 0.4 * limit_phi:
        angle_reward = (0.4*limit_phi) - angle
    else:
        angle_reward = 0.0

    # if (angle <= 0.0 and ballPosition[0] > targetX) or (angle >= 0.0 and ballPosition[0] < targetX):
    #     # angle_reward = abs(targetX - ballPosition[0]) / 100
    #     angle_reward = 0.0
    # else:
    #     # angle_reward = -abs(targetX - ballPosition[0]) / 100
    #     angle_reward = 0.0

    # if time_spent_around_target > 2.0:
    #     proximity_reward = 5.0 #time_spent_around_target / 1.5
    # else:
    #     proximity_reward = 0.0


    # Total reward
    total_reward = distance_reward + time_reward + angle_reward + proximity_reward
...
rangeOfTargetProximityPts = 2.0 # k*ballSize

## AI/ML Rewards ##
enableLearning = True
iterations_to_collect_data = 10
update_period = 0.1 # Limit predictions frequency 
targetX = 0.0
...
def render(time):
    ...
    global showStartingInfo, sim_iteration, data, total_distance_covered, simulation_start_time
    global time_since_last_prediction, total_reward_per_iteration, max_angle, time_spent_around_target
    ...
    if(ballPosition[0] <= targetX + rangeOfTargetProximityPts*ballSize
        and ballPosition[0] >= targetX - rangeOfTargetProximityPts*ballSize):
        time_spent_around_target += deltaTime
    else:
        time_spent_around_target = 0.0

    if time_since_last_prediction >= update_period:
        # Calculate reward based on the current state
        reward = calculate_reward(distance_to_target_x, local_time, phi)
        total_reward_per_iteration += reward
        ...
        target_phi = max(min(target_phi, limit_phi), -limit_phi)
        if(enableLearning):
