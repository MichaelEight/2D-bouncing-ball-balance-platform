...
def render(time):
    ...
    global deltaTime, lastTime, timeSinceLastHit
    ...
    # Define the amplitude and period for the oscillation
    amplitude = 15.0  # The maximum deviation from the center
    period = 4.0  # The time taken for one complete cycle (back and forth)

    # Calculate the angular frequency
    angular_frequency = 2 * pi / period

    # Calculate the target angle based on the sinusoidal oscillation
    target_phi = amplitude * sin(angular_frequency * time)
    ...
    if(checkForCollision):
        ...
        # Check if the distance is less than the radius of the ball also if it actually hits the line, not goes by it
        if distance <= ball_radius + lineWidth/2 and distanceFromP1 <= L and distanceFromP2 <= L and time - timeSinceLastHit >= 0.5:
            # Collision detected, handle it here
            timeSinceLastHit = time
            if distance > 0.005:
                # print("Collision detected!")
                # ballVelocity[1] *= -1
                # print("Old velocity: ", ballVelocity)
    ...
    phi = smooth_transition(phi, target_phi, time, transition_duration)
